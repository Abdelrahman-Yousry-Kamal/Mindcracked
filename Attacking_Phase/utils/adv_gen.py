...
def verify_trajectory(args, model):
    ...
    for idx in range(1, args.num_trajectories + 1):
        ...
        tra_log = torch.load(os.path.join(save_dir, "tra_log.pth"), map_location=args.device, weights_only=False)
        ori_pred = torch.load(os.path.join(save_dir, "pred_log.pth"), map_location=args.device, weights_only=False)
        ...
        mismatches += (tra_pred != ori_pred).sum().item()
        
        if mutation < args.threshold:
            count_pos += 1
            
    detection_rate = count_pos / args.num_trajectories
    overall_mutation = mismatches / total_samples_checked if total_samples_checked > 0 else 0
