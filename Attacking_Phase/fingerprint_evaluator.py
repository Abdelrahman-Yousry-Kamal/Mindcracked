...
def verify_pure_blackbox_trajectory(args, raw_checkpoint):
    ...
    for idx in range(1, args.num_trajectories + 1):
        ...
        tra_log = torch.load(os.path.join(save_dir, "tra_log.pth"), map_location=device, weights_only=False)
        ori_pred = torch.load(os.path.join(save_dir, "pred_log.pth"), map_location=device, weights_only=False)
    ...
    return detection_rate, overall_mutation
...
def main():
    ...
    parser.add_argument('--verify_target', type=str, default='./attacks',
    ...
    if args.mode == 'allocate':
        ...
        source_model.load_state_dict(torch.load(weight_path, map_location=args.device, weights_only=False))
        ...
        for file_idx, filepath in enumerate(suspect_files, 1):
            ...
            try:
                ...
                suspect_model.load_state_dict(torch.load(filepath, map_location=args.device, weights_only=False))
                ...
                raw_checkpoint = torch.load(filepath, map_location=args.device, weights_only=False)
                if isinstance(raw_checkpoint, torch.nn.Module):
                    from utils.adv_gen import verify_trajectory
                    print('    -> Full Model Object Detected. Bypassing dictionary parser...')
                    verify_trajectory(args, raw_checkpoint)
                else:
